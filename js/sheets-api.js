...
const PRODUCTS_RANGE = `${CONFIG.PRODUCTS_SHEET}!A:O`;
...
function rowToProduct(r) {
  return { id:r[0]||'', sku:r[1]||'', name:r[2]||'', category:r[3]||'', material:r[4]||'', weight:r[5]||'',
    gemType:r[6]||'-', gemWeight:r[7]||'-', costPrice:Number(r[8]||0), sellPrice:Number(r[9]||0),
    stock:Number(r[10]||0), status:r[11]||'พร้อมขาย', imageUrl:r[12]||'', notes:r[13]||'-', createdAt:r[14]||'' };
}

function productToRow(p) {
  return [p.id,p.sku,p.name,p.category,p.material,p.weight,p.gemType,p.gemWeight,p.costPrice,p.sellPrice,p.stock,p.status,p.imageUrl,p.notes,p.createdAt];
}
...
async function sheetsUpdateProduct(product) {
  ...
  await gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: CONFIG.SHEET_ID, range, valueInputOption: 'USER_ENTERED', resource: { values: [productToRow(product)] } });
}
...
async function sheetsUpdateStock(sku, newStock) {
  ...
  await gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: CONFIG.SHEET_ID, range, valueInputOption: 'USER_ENTERED', resource: { values: [[newStock, status]] } });
}
...
async function sheetsDeleteProduct(sku) {
  ...
  if (!rowNum) throw new Error(`ไม่พบ SKU: ${sku}`);
  const range = `${CONFIG.PRODUCTS_SHEET}!A${rowNum}:O${rowNum}`;
  await gapi.client.sheets.spreadsheets.values.clear({ spreadsheetId: CONFIG.SHEET_ID, range });
}
...
async function ensureProductsHeader() {
  ...
  if (!resp.result.values || resp.result.values.length === 0) {
    await gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: CONFIG.SHEET_ID, range: `${CONFIG.PRODUCTS_SHEET}!A1:O1`, valueInputOption: 'RAW',
      resource: { values: [['id','sku','name','category','material','weight','gemType','gemWeight','costPrice','sellPrice','stock','status','imageUrl','notes','createdAt']] } });
  }
}
...
async function sheetsAddSale(sale) {
  ...
  await gapi.client.sheets.spreadsheets.values.append({
    spreadsheetId: CONFIG.SHEET_ID, range: `${CONFIG.SALES_SHEET}!A:A`,
    valueInputOption: 'USER_ENTERED', insertDataOption: 'INSERT_ROWS',
    resource: { values: [[sale.id, sale.date, sale.sku, sale.productName, sale.qty, sale.sellPrice, sale.costPrice, sale.profit]] }
  });
}
...
async function ensureSalesHeader() {
  ...
  if (!resp.result.values || resp.result.values.length === 0) {
    await gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: CONFIG.SHEET_ID, range: `${CONFIG.SALES_SHEET}!A1:H1`, valueInputOption: 'RAW',
      resource: { values: [['id','date','sku','productName','qty','sellPrice','costPrice','profit']] } });
  }
}
